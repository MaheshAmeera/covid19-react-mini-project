...
class SpecificState extends Component {
  ...
  renderBarChart = data1 => {
    const {isConfirmActive, isActiveActive, isRecoverActive, isDeceasedActive} =
      this.state
      ...
      >
        ...
        <Tooltip
          ...
          labelFormatter={value =>
            `${new Date(value).getDate()} ${new Date(value).toLocaleString(
              'default',
              {month: 'long'},
            )}`
          }
  ...
  renderSpecificStateView = () => {
    ...
    const {name, active, confirmed, recovered, deceased, tested} =
      specificData[0]
    return (
      <>
